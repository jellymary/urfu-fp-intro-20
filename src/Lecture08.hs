...
dequeue (Queue left []) = (head right, Queue [] (tail right)) where
  right = foldl (\r a -> a : r) [] left
dequeue (Queue left (x:xs)) = (x, Queue left xs)

isEmpty :: Queue a -> Bool
isEmpty (Queue left right) = (length left == 0) && (length right == 0)
...
instance IntArray [Int] where
  fromList xs = map (\(_,a) -> a) xs
  toList xs = zip ([i | i <- [0..(length xs - 1)]]) xs
  update xs index b = (take index xs) ++ (b : drop (index + 1) xs)
  (#) xs index = xs !! index

instance IntArray (Array Int Int) where
  fromList xs = array(0, maximum (map (\(i,_) -> i) xs)) xs
  toList = assocs
...
instance IntArray (Map.IntMap Int) where
  ...
  toList = Map.toList
  update xs index x = Map.update (\_ -> Just x) index xs
  (#) = (Map.!)

-- Сортирует массив целых неотрицательных чисел по возрастанию
countingSort :: forall a. IntArray a => [Int] -> [Int]
countingSort [] = []
countingSort xs = result where
  auxArr = fromList @a ([(i, 0) | i <- [0..(maximum xs)]])
  counts = foldl (\arr x -> update arr x ((arr # x) + 1)) auxArr xs
  result = foldr (\(i,n) x -> (replicate n i) ++ x) [] (toList counts)
