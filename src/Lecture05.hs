...
inHowManyYearsChinaWins = length (takeWhile (\(x, y) -> x < y) (zip chinaGDP usaGDP)) + 1
  where
...
stat events = map (\(Country name count) -> Country name (newCount name events + count)) allCountries
  where newCount name countries = sum (map (\(Country n v) -> if n == name then v else 0) countries)
